    ...
    useEffect(() => {
        ...
        if (window.fleetbo && typeof window.fleetbo.fleetboLog === 'function') {
            if (window.Fleetbo) {
                ...
                requester = setInterval(() => { if (window.Fleetbo) onFleetboReady(); }, 100);
            }
        }
            ...
            requester = setInterval(() => {
             ...
             setTimeout(() => {
                 if (!isReadyRef.current) { window.location.href = 'https://fleetbo.io/docs'; setInitializationError("Running outside of Fleetbo Environment."); }
             }, 2000);
        }
        timer = setTimeout(() => { if (!isReadyRef.current) { setInitializationError("Connection Timeout.");  cleanup(); }}, 15000); 

        return cleanup;
    // eslint-disable-next-line
    }, []); 
    useEffect(() => {  window.navigateToTab = (route) => { navigate(route);}; }, [navigate]);
    useEffect(() => {
        if(isFleetboReady) {
